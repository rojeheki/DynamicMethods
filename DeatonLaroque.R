...
setDiscretization = function(n,rho){
  d$T=generateTransitionMatrix(n,rho)
  d$Z=calculateZLevels(n)
  d$n=n
  d$rho=rho
}
...
for (i in 1:200) {
  j=1
  while (r[i] > d$T[lvls[i],j]) {
    r[i] = r[i]-d$T[lvls[i],j]
    j = j+1
  }
  lvls[i+1] = j
}
plot(1:201, d$Z[lvls], type="l")
